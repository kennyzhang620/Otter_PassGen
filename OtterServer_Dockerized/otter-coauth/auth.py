...
def preauth():
    ...
    if (not A['hash'] or A['hash'] != stime() or (request.remote_addr != "127.0.0.1" and request.remote_addr != "localhost" and request.remote_addr != HOST_IP)) :
        return "FAIL"
...
def auth():
    ...
    if (not authKey[0][5] or authKey[0][5] < (dt.datetime.now().timestamp() - 3) or (request.remote_addr != "127.0.0.1" and request.remote_addr != "localhost" and request.remote_addr != HOST_IP)  ):
        authKey[0] = None
        return "FAIL"
